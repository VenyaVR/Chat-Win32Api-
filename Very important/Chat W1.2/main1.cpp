	...
	{	
		case WM_CREATE: 
		{
			GEditBox = CreateWindow("EDIT", "Enter your message", 
				WS_CHILD | WS_VISIBLE | WS_BORDER | ES_MULTILINE | WS_DISABLED,
...
BOOL CALLBACK DialogFunction(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	static short nCounterOpenDialog = 0;
	...
	switch(message)
	{
		...
		{	
			...
			if(!nCounterOpenDialog)
			{
				SendMessage(  hIP, WM_SETTEXT, 0, (LPARAM)       "IP");
				SendMessage(hPort, WM_SETTEXT, 0, (LPARAM)     "Port");
				SendMessage(hName, WM_SETTEXT, 0, (LPARAM)"Your name");
			}
			else
			{
				SendMessage(  	  hIP, WM_SETTEXT, 0, (LPARAM)      IP);
